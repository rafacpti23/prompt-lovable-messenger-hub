...
serve(async (req) => {
  ...
  try {
    ...
    console.log("🔑 QStash Token configured:", QSTASH_TOKEN ? "Yes" : "No");
    ...
    console.log(`📊 Iniciando processamento QStash com limite de ${MAX_MESSAGES_PER_RUN} mensagens...`);

    // Buscar campanhas agendadas com método QStash
    ...
    if (campaignsError) {
      console.error("❌ Erro ao buscar campanhas QStash:", campaignsError);
      throw new Error(`Failed to fetch QStash campaigns: ${campaignsError.message}`);
    }

    if (!campaigns || campaigns.length === 0) {
      console.log("📭 Nenhuma campanha QStash agendada para processar.");
      return new Response(JSON.stringify({ message: 'Nenhuma campanha QStash agendada.' }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }

    console.log(`🎯 Encontradas ${campaigns.length} campanhas QStash para processar`);

    for (const campaign of campaigns) {
      if (totalProcessed >= MAX_MESSAGES_PER_RUN || (Date.now() - startTime) > MAX_PROCESSING_TIME) {
        console.log("⏰ Limite de tempo/mensagens atingido");
        break;
      }
      ...
      for (let i = 0; i < contacts.length; i++) {
        ...
        const qstashEndpoint = `${QSTASH_URL}/v2/publish/${encodeURIComponent(campaign.qstash_webhook_url || 'https://webhook.site/default')}`;
