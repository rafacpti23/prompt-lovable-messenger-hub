...
serve(async (req) => {
  ...
  try {
    ...
    if (!messages || messages.length === 0) {
      return new Response(JSON.stringify({ message: "Nenhuma mensagem na fila." }), {
    ...
    return new Response(JSON.stringify({ message: `Processamento concluído para ${messages.length} mensagens.` }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
